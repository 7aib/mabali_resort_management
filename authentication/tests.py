...
from authentication.views import _validate_phone_number, _check_phone_number_exists
...
class UserModelTest(TestCase):
    ...
    def test_all_role_values_exist(self):
        expected_roles = [
            'CASHIER', 'ACCOUNTANT', 'MAIN_CASHIER', 'CEO', 'HOST',
            'SALE', 'CUSTOMER', 'DRIVER', 'WAITER', 'HR_MANAGER',
        ]
...
class LoginViewTest(TestCase):
    ...
    def test_post_invalid_password(self):
        ...
        self.assertTrue(any('Invalid username or password' in str(m) for m in messages_list))
...
class LogoutViewTest(TestCase):
    ...
    def test_logout_logs_out_user(self):
        ...
        response = self.client.get(reverse('profile'))
        self.assertRedirects(response, reverse('login') + '?next=/profile/', fetch_redirect_response=False)
...
class ChangePasswordViewTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, reverse('login') + '?next=/change-password/', fetch_redirect_response=False)
...
class EmployeeDashboardTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, reverse('login') + '?next=/employee_dashboard/', fetch_redirect_response=False)
...
class EmployeeCreateTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, '/?next=/employee_create/', fetch_redirect_response=False)
    ...
    def test_create_employee_duplicate_username(self):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertTrue(any('already exists' in str(m).lower() or 'Error' in str(m) for m in messages_list))
...
class EmployeeDetailTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, '/?next=/employee/%d/' % self.user.pk, fetch_redirect_response=False)
...
class EmployeeEditTest(TestCase):
    ...
    def setUp(self):
        ...
        self.employee = User.objects.create_user(
            username='editemp', password='TestPass123!',
            role=UserRoles.CASHIER, phone_number='+923011234567'
        )

    def test_requires_login(self):
        response = self.client.get(reverse('employee_edit', args=[self.employee.pk]))
        self.assertRedirects(response, '/?next=/employee/%d/edit/' % self.employee.pk, fetch_redirect_response=False)
    ...
    def test_edit_updates_employee(self):
        ...
        self.assertRedirects(response, reverse('employee_detail', args=[self.employee.pk]), fetch_redirect_response=False)
...
class EmployeeDeleteTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, '/?next=/employee/%d/delete/' % self.employee.pk, fetch_redirect_response=False)
...
class IntegrationTest(TestCase):
    ...
    def test_full_employee_lifecycle(self):
        ...
        # Edit
        response = self.client.post(reverse('employee_edit', args=[emp.pk]), {
            'first_name': 'Updated',
            'last_name': 'Name',
            'email': 'updated@test.com',
            'role': UserRoles.ACCOUNTANT,
