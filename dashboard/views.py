...
from inventory.models import FuelTransactionLog, AmmoTransactionLog, AmbulanceLog
...
def dashboard_view(request: HttpResponse) -> HttpResponse:
    ...
    if user_role == UserRoles.CASHIER:
        cashier_entries = today_entries.filter(
            created_by=request.user
        ).select_related('customer').order_by('-created_at')[:10]
        cashier_transactions = EntryTransaction.objects.filter(
            created_at__date=today,
            entry_form__created_by=request.user
        )
        cashier_total_collected = cashier_transactions.aggregate(s=Sum('amount'))['s'] or 0
        ...
        today_ambulance_logs = AmbulanceLog.objects.filter(
            date=today
        ).select_related('ambulance', 'driver').order_by('-created_at')[:10]
        ...
        today_free_bills = FreeBilling.objects.filter(
            date=today
        ).order_by('-created_at')[:10]
        free_bill_count = FreeBilling.objects.filter(date=today).count()
        free_bills_total = FreeBilling.objects.filter(date=today).aggregate(
    ...
    if is_admin:
        ...
        new_guests = today_entries.filter(status='New').count()
        old_guests = today_entries.filter(status='Old').count()
        guest_entries_detail = today_entries.select_related('customer').order_by('-created_at')
