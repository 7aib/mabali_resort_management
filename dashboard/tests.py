...
from cash_counters.constants import CitiesChoices
from cash_counters.constants import CounterTypeChoices as CCCounterType
from cash_counters.constants import GateChoices
from cash_counters.constants import PaymentMethodChoices as CCPaymentMethod
...
from finance.constants import CounterTypeChoices as FinCounterType
from finance.constants import POSPaymentMethodChoices
from finance.models import POS
from inventory.choices import AssetCategoryChoices, StockStatusChoices
...
class DashboardRoomStatsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.room = Room.objects.create(
            name="Hut 1",
            category=RoomCategoryChoices.HUT,
            rate_per_night=Decimal("10000.00"),
        )
        self.room2 = Room.objects.create(
            name="Suite 1",
            category=RoomCategoryChoices.SUITE,
...
class DashboardAllTimeStatsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.room = Room.objects.create(
            name="Room All",
            category=RoomCategoryChoices.ROOM,
        )
...
class DashboardEventsTest(TestCase):
    ...
    def test_decor_team_event_appears(self):
        EntryCounterForm.objects.create(
            customer=self.customer,
            no_of_persons=5,
            visit_type=VisitTypeChoices.DECOR_TEAM,
            gate=GateChoices.EVENT,
            status=StatusChoices.NEW,
        )
        ctx = self.client.get(reverse("dashboard")).context
