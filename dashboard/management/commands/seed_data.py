...
from reservations.choices import ReservationStatusChoices, PaymentTypeChoices
from inventory.models import InventoryItem, FuelTransactionLog, AmmoTransactionLog
from inventory.choices import AssetCategoryChoices, StockStatusChoices, AmmoPaymentChoices
from cash_counters.models import EntryCounterForm, EntryTransaction, TicketRefund
from cash_counters.constants import CitiesChoices, VisitTypeChoices, GateChoices, StatusChoices, PaymentMethodChoices
from finance.models import POS
from finance.constants import POSPaymentMethodChoices, CounterTypeChoices
    ...
    def handle(self, *args, **options):
        ...
        for name, category, rate in rooms_data:
            Room.objects.get_or_create(
                name=name,
                defaults={'category': category, 'rate_per_night': rate}
            )
        ...
        for i in range(3):
            POS.objects.get_or_create(
                date=today,
                counter_type=CounterTypeChoices.MAIN_GATE if i == 0 else CounterTypeChoices.RESTAURANT,
                amount=15000 + (i * 5000),
                defaults={
