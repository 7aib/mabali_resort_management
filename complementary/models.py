...
class FreeBilling(TimeStampedModelMixin, SoftDeleteModelMixin, models.Model):
    ...
    bill_type = models.CharField(
        max_length=50, choices=BillTypeChoices.choices,
        default=BillTypeChoices.QB_BILL
    )
    head = models.CharField(
        max_length=100, choices=HeadChoices.choices,
        default=HeadChoices.GUEST
    )
    bill_status = models.CharField(
        max_length=50, choices=BillStatusChoices.choices,
        default=BillStatusChoices.PAID
    )
    invoice_no = models.CharField(max_length=50, blank=True, default='')
    department = models.CharField(
        max_length=50, choices=DepartmentChoices.choices,
        default=DepartmentChoices.ALL
    )
    ...
    def __str__(self):
        return '%s — %s — Rs. %s on %s' % (
            self.get_head_display(), self.guest_name or '—', self.total_bill_amount, self.date
        )
