...
def generator_log_view(request):
    ...
    generators = InventoryItem.objects.filter(category="generator", is_deleted=False)
    ...
    # Get today's logs
    today_logs = (
        GeneratorLog.objects.filter(created_at__date=today)
        .select_related("generator", "created_by")
        .order_by("-created_at")
    )
...
def ambulance_log_view(request):
    ...
    ambulances = InventoryItem.objects.filter(category="ambulance", is_deleted=False)

    # Get drivers (users with relevant roles)
    drivers = User.objects.filter(is_active=True).order_by("first_name", "username")
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or not patient_type
            or not ambulance_id
            or not start_reading_km
            or not end_reading_km
            or not hospital
            or not driver_id
        ):
            messages.error(request, "All required fields must be filled.")
...
def fuel_entry_view(request):
    ...
    fuel_items = InventoryItem.objects.filter(category="fuel", is_deleted=False)
    ...
    today_entries = (
        FuelTransactionLog.objects.filter(date=today)
        .select_related("inventory_item", "created_by", "issued_to")
        .order_by("-created_at")
    )
...
def ammo_entry_view(request):
    ...
    ammo_items = InventoryItem.objects.filter(category="ammo", is_deleted=False)
    ...
    if request.method == "POST":
        ...
        if transaction_status == StockStatusChoices.ISSUED and not payment:
            messages.error(
                request, "Payment method is required for issued transactions."
            )
            return redirect("inventory:ammo_entry")
        ...
        try:
            AmmoTransactionLog.objects.create(
                ...
                payment=(
                    payment if transaction_status == StockStatusChoices.ISSUED else ""
                ),
    ...
    today_entries = (
        AmmoTransactionLog.objects.filter(date=today)
...
@login_required
@roles_required(
    UserRoles.CEO, UserRoles.ACCOUNTANT, UserRoles.HR_MANAGER, UserRoles.MAIN_CASHIER
)
@log_errors
def purchase_orders_view(request):
    fuel_ordered = (
        FuelTransactionLog.objects.filter(transaction_status=StockStatusChoices.ORDERED)
        .select_related("inventory_item", "created_by")
        .order_by("-date")
    )

    ammo_ordered = (
        AmmoTransactionLog.objects.filter(transaction_status=StockStatusChoices.ORDERED)
        .select_related("inventory_item", "created_by")
        .order_by("-date")
    )

    fuel_required = (
        FuelTransactionLog.objects.filter(
    ...
    ammo_required = (
        AmmoTransactionLog.objects.filter(
            transaction_status=StockStatusChoices.REQUIRED
        )
        .select_related("inventory_item", "created_by")
        .order_by("-date")
    ...
    if request.method == "POST":
        ...
        if model_type == "fuel":
            try:
                ...
                messages.success(
                    request,
                    "Fuel transaction updated to %s."
                    % record.get_transaction_status_display(),
                )
...
def stock_management_view(request):
    fuel_items = InventoryItem.objects.filter(
        category="fuel", is_deleted=False
    ).order_by("name")
    ammo_items = InventoryItem.objects.filter(
        category="ammo", is_deleted=False
    ).order_by("name")
    generator_items = InventoryItem.objects.filter(
        category="generator", is_deleted=False
    ).order_by("name")
    ambulance_items = InventoryItem.objects.filter(
        category="ambulance", is_deleted=False
    ).order_by("name")
