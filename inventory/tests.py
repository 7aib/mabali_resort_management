...
from .models import (
    InventoryItem, FuelTransactionLog, AmmoTransactionLog,
    GeneratorLog, AmbulanceLog,
)
from .choices import (
    AssetCategoryChoices, StockStatusChoices, HospitalChoices,
    PatientTypeChoices, AmmoPaymentChoices,
)
from .utils import (
    STATUS_TRANSITIONS, validate_status_transition,
    get_creation_statuses, get_allowed_transitions,
)
...
class StatusTransitionsTest(TestCase):
    ...
    def test_required_transitions_to_issued(self):
        self.assertIn(StockStatusChoices.ISSUED, STATUS_TRANSITIONS[StockStatusChoices.REQUIRED])

    def test_required_transitions_to_ordered(self):
        self.assertIn(StockStatusChoices.ORDERED, STATUS_TRANSITIONS[StockStatusChoices.REQUIRED])

    def test_required_transitions_to_cancelled(self):
        self.assertIn(StockStatusChoices.CANCELLED, STATUS_TRANSITIONS[StockStatusChoices.REQUIRED])

    def test_ordered_transitions_to_purchased(self):
        self.assertIn(StockStatusChoices.PURCHASED, STATUS_TRANSITIONS[StockStatusChoices.ORDERED])

    def test_ordered_transitions_to_cancelled(self):
        self.assertIn(StockStatusChoices.CANCELLED, STATUS_TRANSITIONS[StockStatusChoices.ORDERED])
...
class ValidateStatusTransitionTest(TestCase):
    ...
    def test_valid_transition_required_to_ordered(self):
        result = validate_status_transition(StockStatusChoices.REQUIRED, StockStatusChoices.ORDERED)
        self.assertTrue(result)

    def test_valid_transition_required_to_issued(self):
        result = validate_status_transition(StockStatusChoices.REQUIRED, StockStatusChoices.ISSUED)
        self.assertTrue(result)

    def test_valid_transition_required_to_cancelled(self):
        result = validate_status_transition(StockStatusChoices.REQUIRED, StockStatusChoices.CANCELLED)
        self.assertTrue(result)

    def test_valid_transition_ordered_to_purchased(self):
        result = validate_status_transition(StockStatusChoices.ORDERED, StockStatusChoices.PURCHASED)
        self.assertTrue(result)

    def test_valid_transition_ordered_to_cancelled(self):
        result = validate_status_transition(StockStatusChoices.ORDERED, StockStatusChoices.CANCELLED)
        self.assertTrue(result)

    def test_same_status_returns_true(self):
        result = validate_status_transition(StockStatusChoices.REQUIRED, StockStatusChoices.REQUIRED)
        self.assertTrue(result)

    def test_invalid_transition_ordered_to_issued(self):
        with self.assertRaises(ValidationError):
            validate_status_transition(StockStatusChoices.ORDERED, StockStatusChoices.ISSUED)

    def test_invalid_transition_issued_to_purchased(self):
        with self.assertRaises(ValidationError):
            validate_status_transition(StockStatusChoices.ISSUED, StockStatusChoices.PURCHASED)

    def test_invalid_transition_cancelled_to_any(self):
        with self.assertRaises(ValidationError):
            validate_status_transition(StockStatusChoices.CANCELLED, StockStatusChoices.ISSUED)

    def test_invalid_transition_purchased_to_any(self):
        with self.assertRaises(ValidationError):
            validate_status_transition(StockStatusChoices.PURCHASED, StockStatusChoices.ORDERED)
...
class GetAllowedTransitionsTest(TestCase):
    ...
    def test_terminal_returns_empty(self):
        for status in [StockStatusChoices.ISSUED, StockStatusChoices.PURCHASED, StockStatusChoices.CANCELLED]:
...
class InventoryItemModelTest(TestCase):
    ...
    def test_ordering(self):
        item1 = InventoryItem.objects.create(name='B', category=AssetCategoryChoices.OTHER)
        item2 = InventoryItem.objects.create(name='A', category=AssetCategoryChoices.OTHER)
    ...
    def test_soft_delete_fields(self):
        item = InventoryItem.objects.create(name='Test', category=AssetCategoryChoices.OTHER)
...
class FuelTransactionLogModelTest(TestCase):
    ...
    def setUp(self):
        self.fuel = InventoryItem.objects.create(
            ...
            stock_quantity=Decimal('1000.00'), unit='liters',
        )
        self.vehicle = InventoryItem.objects.create(
            name='J1', category=AssetCategoryChoices.JETSKI,
        )
...
class AmmoTransactionLogModelTest(TestCase):
    ...
    def setUp(self):
        self.ammo = InventoryItem.objects.create(
            name='9mm Rounds', category=AssetCategoryChoices.AMMO,
            stock_quantity=Decimal('500'), unit='rounds',
        )
...
class GeneratorLogModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.generator = InventoryItem.objects.create(
            name='100kv Generator', category=AssetCategoryChoices.GENERATOR,
        )
...
class AmbulanceLogModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ambulance = InventoryItem.objects.create(
            name='Ambulance 1', category=AssetCategoryChoices.AMBULANCE,
        )
...
class InventoryItemCreateViewTest(TestCase):
    ...
    def test_requires_login(self):
        ...
        self.assertRedirects(response, reverse('login') + '?next=/inventory/items/create/', fetch_redirect_response=False)
    ...
    def test_post_creates_item(self):
        ...
        self.assertRedirects(response, reverse('inventory:item_list'), fetch_redirect_response=False)
...
class InventoryItemListViewTest(TestCase):
    ...
    def test_requires_login(self):
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/items/', fetch_redirect_response=False)
    ...
    def test_shows_items(self):
        item = InventoryItem.objects.create(name='Fuel', category=AssetCategoryChoices.FUEL)
        response = self.client.get(self.url)
        self.assertIn(item, response.context['items'])

    def test_excludes_deleted_items(self):
        item = InventoryItem.objects.create(name='Deleted', category=AssetCategoryChoices.OTHER, is_deleted=True)
        response = self.client.get(self.url)
        self.assertNotIn(item, response.context['items'])

    def test_search_by_name(self):
        item = InventoryItem.objects.create(name='Petrol 95', category=AssetCategoryChoices.FUEL)
    ...
    def test_search_by_supplier(self):
        item = InventoryItem.objects.create(name='Fuel', category=AssetCategoryChoices.FUEL, supplier='Shell Pakistan')
        response = self.client.get(self.url, {'search': 'Shell'})
        self.assertIn(item, response.context['items'])

    def test_category_filter(self):
        item = InventoryItem.objects.create(name='Petrol', category=AssetCategoryChoices.FUEL)
        InventoryItem.objects.create(name='Ammo Box', category=AssetCategoryChoices.AMMO)
...
class InventoryItemEditViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.item = InventoryItem.objects.create(
            name='Old Name', category=AssetCategoryChoices.FUEL,
        )
...
class InventoryItemDeleteViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.item = InventoryItem.objects.create(
            name='To Delete', category=AssetCategoryChoices.OTHER,
        )
...
class FuelEntryViewTest(TestCase):
    ...
    def test_requires_login(self):
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/fuel-entry/', fetch_redirect_response=False)
    ...
    def test_post_insufficient_stock(self):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertTrue(any('insufficient' in str(m).lower() or 'error' in str(m).lower() for m in messages_list))
...
class AmmoEntryViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ammo = InventoryItem.objects.create(
            name='9mm Ammo', category=AssetCategoryChoices.AMMO,
    ...
    def test_requires_login(self):
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/ammo-entry/', fetch_redirect_response=False)
...
class GeneratorLogViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.generator = InventoryItem.objects.create(
            name='100kv Gen', category=AssetCategoryChoices.GENERATOR,
        )
    ...
    def test_requires_login(self):
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/generator-log/', fetch_redirect_response=False)
...
class AmbulanceLogViewTest(TestCase):
    ...
    def test_requires_login(self):
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/ambulance-log/', fetch_redirect_response=False)
...
class PurchaseOrdersViewTest(TestCase):
    ...
    def test_requires_login(self):
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/purchase-orders/', fetch_redirect_response=False)
    ...
    def test_post_updates_ammo_status(self):
        ...
        ammo = InventoryItem.objects.create(
            name='Ammo', category=AssetCategoryChoices.AMMO, stock_quantity=Decimal('100'),
        )
        tx = AmmoTransactionLog.objects.create(
            inventory_item=ammo,
            transaction_status=StockStatusChoices.REQUIRED,
...
class StockManagementViewTest(TestCase):
    ...
    def test_requires_login(self):
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('login') + '?next=/inventory/stock-management/', fetch_redirect_response=False)
    ...
    def test_shows_all_item_categories(self):
        ...
        InventoryItem.objects.create(name='Gen', category=AssetCategoryChoices.GENERATOR)
        InventoryItem.objects.create(name='Amb', category=AssetCategoryChoices.AMBULANCE)
        response = self.client.get(self.url)
