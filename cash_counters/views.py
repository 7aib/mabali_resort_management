...
def daily_sales_view(request):
    ...
    daily_entries = (
        EntryCounterForm.objects.filter(created_at__date=today)
    ...
    if request.method == "POST":
        ...
        if visit_type == VisitTypeChoices.PAID:
            ...
            EntryTransaction.objects.create(
                entry_form=entry, amount=amount, payment_method=payment_method
            )
...
def cash_handover_view(request):
    ...
    cashiers = User.objects.filter(
        role__in=[UserRoles.CASHIER, UserRoles.MAIN_CASHIER], is_active=True
    ).order_by("first_name", "username")

    main_cashiers = User.objects.filter(
        role=UserRoles.MAIN_CASHIER, is_active=True
    ).order_by("first_name", "username")
    ...
    today_handovers = (
        CashHandover.objects.filter(date=today)
        .select_related("cashier", "handover_to")
        .order_by("-created_at")
    )
...
def cash_register_view(request):
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or not received_from_id
            or not on_duty_cashier_id
        ):
            messages.error(request, "All required fields must be filled.")
    ...
    # Get today's entries
    today_entries = (
        CashRegister.objects.filter(date=today)
        .select_related("received_from", "on_duty_cashier")
        .order_by("-created_at")
    )
...
def ticket_refund_view(request):
    ...
    if request.method == "POST":
        ...
        if (
            not date
            or not no_of_tickets
            or not rate_per_ticket
            or not total_amount_refunded
            or not reason
        ):
            messages.error(request, "All required fields must be filled.")
    ...
    today_refunds = TicketRefund.objects.filter(date=today).order_by("-created_at")
