...
class EntryCounterForm(TimeStampedModelMixin, SoftDeleteModelMixin, models.Model):
    ...
    location = models.CharField(
        max_length=50, choices=CitiesChoices.choices, default=CitiesChoices.OTHER
    )
    no_of_persons = models.PositiveIntegerField(default=1)
    no_of_kids = models.PositiveIntegerField(default=0)
    visit_type = models.CharField(
        max_length=50, choices=VisitTypeChoices.choices, default=VisitTypeChoices.PAID
    )
    gate = models.CharField(
        max_length=50, choices=GateChoices.choices, default=GateChoices.MAIN
    )
...
class EntryTransaction(TimeStampedModelMixin, SoftDeleteModelMixin, models.Model):
    ...
    entry_form = models.OneToOneField(
        EntryCounterForm, on_delete=models.CASCADE, related_name="transaction"
    )
...
class CashRegister(TimeStampedModelMixin, SoftDeleteModelMixin, models.Model):
    ...
    on_duty_cashier = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
...
class TicketRefund(TimeStampedModelMixin, SoftDeleteModelMixin, models.Model):
    ...
    total_amount_refunded = models.DecimalField(
        max_digits=12, decimal_places=2, default=0
    )
    reason = models.CharField(
        max_length=100,
        choices=TicketRefundReasonChoices.choices,
